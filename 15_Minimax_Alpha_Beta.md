...
```text
            [α=3, β=inf]
              ...
              /   \
             /     \
      [α=-inf,β=3]  [α=3, β=1]  <-- PRUNE β(1) ≤ α(3)
          MIN           MIN
         /   \         /   \
       (3)   (5)     (1)   [PRUNED]
```
...
For games with massive branching factors (like **Go**), traditional minimax with alpha-beta is insufficient. MCTS uses random simulations to estimate state values.

### The Four Steps:
1. **Selection:** Starting at the root, navigate down the tree using a selection policy (like **UCB1**) until a leaf node is reached.
2. **Expansion:** If the leaf node doesn't end the game, create one or more child nodes.
3. **Simulation (Rollout):** Perform a random playout from the new node until a terminal state is reached.
4. **Backpropagation:** Use the result of the simulation to update statistics (win/loss) in the nodes along the path to the root.
...
```text
    (1) Selection -> (2) Expansion -> (3) Simulation -> (4) Backprop
       ...
       [Root]           [Node]            [Node]            [Root]
         |                |                 |                 |
       [Best]          [New*]            [Result]          [Update]
         |                |                 |                 |
         v                v                 v                 v
      Navigate         Create            Random            Refine
      to Leaf          Child             Playout           Stats
```

### Advantages:
- **No Evaluation Function Needed:** Only needs game rules to run simulations.
- **Aperiodic:** Can stop at any time and return the best move found so far.
- **Handles High Branching:** Focuses on the most promising branches.
...
**Previous:** [← A* Search](14_A_Star_Search.md) | **Next:** [Constraint Satisfaction Problems →](16_Constraint_Satisfaction_Problems.md)
